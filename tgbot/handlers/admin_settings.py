...
from tgbot.keyboards.inline_admin import turn_open_final, settings_open_final
...
async def settings_data_edit(message: Message, state: FSMContext):
    ...
    await message.answer("<b>🖍 Изменение данных бота.</b>", reply_markup=settings_open_final())
...
async def settings_turn_edit(message: Message, state: FSMContext):
    ...
    await message.answer("<b>🕹 Включение и выключение основных функций</b>", reply_markup=turn_open_final())
...
async def settings_turn_work(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_reply_markup(reply_markup=turn_open_final())
