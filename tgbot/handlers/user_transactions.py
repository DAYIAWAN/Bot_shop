...
from tgbot.keyboards.inline_user import refill_bill_final, refill_choice_final
...
async def refill_get(message: Message, state: FSMContext):
    if message.text.isdigit():
        ...
        if min_input_qiwi <= pay_amount <= 300000:
            ...
            if get_message:
                await cache_message.edit_text(get_message, reply_markup=refill_bill_final(get_link, receipt, get_way))
        else:
