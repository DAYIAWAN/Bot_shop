...
def refill_bill_final(send_requests, get_receipt, get_way):
    keyboard = InlineKeyboardMarkup(
    ).add(
...
def products_open_final(position_id, category_id, remover):
    keyboard = InlineKeyboardMarkup(
    ).add(
...
def products_confirm_final(position_id, get_count):
    keyboard = InlineKeyboardMarkup(
    ).add(
...
# Ссылка на поддержку
def user_support_final(user_name):
    keyboard = InlineKeyboardMarkup(
    ).add(
